...
<ol>
 ...
 <li>SQL automatically generates its best guess as to what the data type is for each column and likely going to interpret "ride_length" as multiple different data types because some sheets have longer than 24-hour durations (which is why normally we'd remove them in a real data analysis stakeholder situation), which complicates things. Our job is to make sure the data type is consistent throughout all 12 files. These are the three you may see and why SQL chose them:
 <ul>
