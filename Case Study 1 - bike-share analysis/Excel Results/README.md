...
 <ol>
   ...
   <li> Select N2 > press CTRL+C > use macros to autofill the column (web search) or Select N2 > press CTRL+C > in N3 hold CTRL+SHIFT+down-arrow key > CTRL+V aka paste, then we need to find the last filled row (hold CTRL+SHIFT+down-arrow key) + 1 select that cell hold CTRL+SHIFT+down-arrow key again and delete the invalid entries (Use PAGE UP & DOWN to move smoothly when close).</li>
